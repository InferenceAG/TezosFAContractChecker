...
export class FA2singlespecific {
  
  async getBalance(fa2contract: FA2, user: string, tokenId: BigNumber): Promise<BigNumber> {
    await fa2contract.updateStorage();
    
    if (await fa2contract.storage.assets.ledger.get(user.toString()) !== undefined) {
      const balance: BigNumber = await fa2contract.storage.assets.ledger.get(user.toString()) as BigNumber;
